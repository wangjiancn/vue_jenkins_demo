...
import { removeRow, updateRow, createRow } from '@/api/cqi'
export default {
  components: { Dialog },
  ...
  methods: {
    ...
    handleDelete(index, row) {
      removeRow(this.model_name, row.id).then(res => {
        console.log(res)
        if (!res.code) {
          this.loadItems()
        }
    ...
    onSubmit() {
      if (this.form.id) {
        updateRow(this.model_name, this.form.id, this.form).then(res => {
          if (!res.code) {
            this.loadItems()
        ...
        createRow(this.model_name, this.form).then(res => {
          if (!res.code) {
            this.loadItems()
            this.popupVisable = false
          }
